    ...
    def get(self, request):
        try:
            user_reservation = Reservation.objects.get(
                customer=request.user, checked_out=False)
            if user_reservation:
                messages.warning(
                    self.request, 'Please Check Out Your Previous Reservation')
...
class Ticket_Pdf(LoginRequiredMixin, View):
    def get(self, request):

        today = timezone.now()
        reservation = Reservation.objects.filter(Q(customer=request.user, checked_out=False) | Q(
            customer=request.user, checked_out=True)).first()
        if reservation:
            params = {
                'today': today,
            ...
            messages.warning(
                self.request, f'No Parking reservation exists for {self.request.user}')
            return redirect('index')


class Display_Tickets(LoginRequiredMixin, View):
    def get(self, request):
...
def check_out(request):
    try:
        ...
        messages.warning(
            request, f'No Parking reservation exists for {request.user}')
